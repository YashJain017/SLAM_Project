...
class MarkovChain:
    def __init__(self):
        ...
        with open('/home/rhea/workspace/catkin_ws/src/SLAM_Project/scripts/model_path1_15_test.pkl','rb') as f:
    ...
    def pose_callback(self, pose_msg):
        ...
        if ((abs(self.previousPosesBuffer[-1][0]- PositionX) >= 0.1) or (abs(self.previousPosesBuffer[-1][1]- PositionY) >= 0.1) or (abs(self.previousPosesBuffer[-1][2]- Yaw) >= 0.1)):
            df=pd.read_csv(r"/home/rhea/workspace/catkin_ws/src/SLAM_Project/scripts/path1_15combined.csv")
            df1=df.copy()
        ...
        # print('the prediction:', self.predictedPose)
    ...
    def sample_next(self, ctx, model, k):
        ...
        if model.get(ctx) is None:
            # print('prev:', self.predictedPose[0], self.predictedPose[1], self.predictedPose[2])
        ...
        #print('buffer', ctx)
        self.counter+=1
        print('Prediction:', possible_Chars[idx],self.counter)
